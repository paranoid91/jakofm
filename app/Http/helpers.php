...
if(!function_exists('checkAuthorCats'))
{
    function checkAuthorCats($id, $cats)
    {
       if(count($cats)>0)
       {
           foreach($cats as $cat)
...
if(!function_exists('getAudioCats'))
{
    function getAudioCats($type, $files)
    {
        $cats = trans('front.'.$type);

        if(count($files)>0)
        {
           foreach($files as $file)
           {
               $cats .= ','.trans('front.'.$file->audio_type);
           }
        }
...
if(!function_exists('audioTypesList'))
{
    function audioTypesList($file, $types)
    {
        $key = array_search($file->audio_type,$types);
        $types[$key] = [
            'id' => $file->id,
...
if(!function_exists('resetTypes'))
{
    function resetTypes()
    {
        return ['artistic','academic','commercial','vocal'];
    }
}

if(!function_exists('printOptions'))
{
    function printOptions($options, $tabName = false)
    {
        $output = [];
        if(count($options)>0)
        {
            ...
            {
                $output[$option->id] = ($tabName == true) ? $option->tab_name : $option->page_name;
            }
...
if(!function_exists('removeEmptyValue'))
{
    function removeEmptyValue($array)
    {
        ...
        {
            $value = trim($value);
            if (empty($value) || strlen($value)<1 || $value == "")
            {
                unset($array[$key]);
            }
        }
...
if(!function_exists('getFiles'))
{
    function getFiles($files)
    {
        ...
        if (!empty($files))
        {
            foreach($files as $file)
            {
                $output_files[$file->type][] = $file->path;
            }
